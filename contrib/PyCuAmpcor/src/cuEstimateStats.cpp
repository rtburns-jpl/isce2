...
void cuEstimateVariance(cuArrays<float> *corrBatchRaw, cuArrays<int2> *maxloc, cuArrays<float> *maxval, int templateSize, cuArrays<float3> *covValue, hipStream_t stream)
{
    ...
    hipLaunchKernelGGL(cudaKernel_estimateVar, dim3(IDIVUP(size, NTHREADS)), dim3(NTHREADS), 0, stream, corrBatchRaw->devData, corrBatchRaw->height, corrBatchRaw->width, maxloc->devData, maxval->devData, templateSize, covValue->devData, size);
